...
function installServiceWorker() {
    return Promise.all([
        caches.open(CACHE_VERSIONS.assets).then(
            (cache) => cache.addAll(BASE_CACHE_FILES)
        ),
        caches.open(CACHE_VERSIONS.offline).then(
            (cache) => cache.addAll(OFFLINE_CACHE_FILES)
        ),
        caches.open(CACHE_VERSIONS.notFound).then(
            (cache) => cache.addAll(NOT_FOUND_CACHE_FILES)
        )
    ]).then(
        () => self.skipWaiting(),
        (err) => console.error(`Couldn't install worker: ${err.stack}`)
    )
}
